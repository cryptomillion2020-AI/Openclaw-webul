/**
 * capture-dashboard-canon.cjs — approval renderings for Agent Comms.
 *
...
(async () => {
  ...
  for (const [name, q] of [['A-quiet', ''], ['B-loaded', '?state=loaded']]) {
    ...
    await page.goto(`${BASE}/capture-network.html${q}`, { waitUntil: 'networkidle' });
    ...
    await page.screenshot({ path: path.join(OUT, `network-${name}.png`) });
