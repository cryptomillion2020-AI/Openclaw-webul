...
export function createTradingEdge({ config = accessConfig, verify = createAccessVerifier(config), fetchOrigin = (...args) => fetch(...args), staticAssets = true, bridge = bridgeWebSocket } = {}) {
  ...
  return {
    async fetch(request, env = {}) {
      ...
      } else if (!((['/api/trading/snapshot','/api/trading/paper/status','/api/trading/paper/ledger'].includes(url.pathname) && request.method === 'GET') || (['/api/trading/preflight','/api/trading/paper/approval','/api/trading/paper/submit','/api/trading/paper/cancel','/api/trading/paper/reduce','/api/trading/paper/settle','/api/trading/paper/policy/validate'].includes(url.pathname) && request.method === 'POST'))) {
      ...
      const keep = new Set(['origin','content-type','content-length','accept','idempotency-key','upgrade','connection','sec-websocket-key','sec-websocket-version','sec-websocket-protocol','sec-websocket-extensions',ASSERTION_HEADER]);
